...
### Launch
The satellite liftoff to deployment on the SpaceX Falcon9 rocket completes in 65 minutes. In the last decade, satellite launch transformed from a magical event to a routine deployment. In cost terms, SpaceX slashed launch costs at least ten times (projected to be 1000x) compared to legacy technology by reusing rocket engines and top fairings. A Falcon9 launch is now estimated to cost $30 million. The 70m tall Falcon9 launches is viewed as two stages:

![Falcon9](../assets/images/falcon9.png)

_Source: https://asd.gsfc.nasa.gov/archive/tess/launch.html (edited for clarity)_
#### Stage1
The 60 Starlink satellites are powered down and stackedd in the top one-third fairing of the Falcon9 in the above picture. The bottom two-third is the booster engine which carries the payload of satellites to 65 km altitude.

On completing a successful stage 1 launch, the booster returns to Earth for reuse. SpaceX’s reusable launch system technology for the first booster stage saves all nine engines. The boosters are reused up to nine times. After taking additional fuel, refurbishment (10%), and payload reduction (under 40%) into consideration, SpaceX brings rocket launch cost down to $30 million. The rocket payload approximately costs $1,350/kg. For comparison, international shipping rates are approximately $15/kg. See Appendix A for launch costs. The low cost satellite launch is a two pronged win with global internet in the short-term and commercial space travel in the long-term.

Coming back to stage 1, the process completes with the main engine cutoff in about two and half minutes counting from the launch time. The booster returns to Earth with main engines while Stage 2 heads to orbit.

#### Stage 2
The Stage 2 head is a clamshell-like “fairing” designed to protect smallsats from launch time excessive heat and pressure. The fairing shell peels and falls down to Earth at the beginning of this stage. A solo engine carries the payload to orbit. The Stage 2 engine takes 35 minutes to get to orbit and park Starlink satellites. About 64 minutes from launch time, the rocket deploys smallsats into their respective orbits.

### Join
The orbiting smallsats take 2 weeks to distance themselves and take respective orbital positions. Each smallsat is configured to occupy a position using: Orbit’s height from sea level, the angle from the equator, and angle from a fixed space direction Smallsat’s angle from a fixed space direction The smallsats are launched initially into a lower orbit. As orbital velocity decreases with altitude, the smallsat changes velocity to match with the target orbital plane. On arriving into the position, the smallsat will lock velocity. As the count of satellites locked into position increases, the constellation size increases. The constellation formation model is nicely depicted in {% include video id="rddTXl_7Wr8" provider="youtube" %}

### Run
Once in orbit, a satellite becomes a constellation member. Its mission is to route packets from the terrestrial network, forward to satellites using laser crosslinks, and beam back to the terrestrial network.

Packets are forwared from the edge device to the Spacelink user terminal, which is a pizza-sized dish antenna. Packets are encrypted at the user terminal before beaming to the smallsat. The first recipient smallsat inspects the destination address. It routes to another smallsat in the packet direction. The last satellite closer to the destination will beam the packet down to all user terminals in coverage area. Only one user terminal will have a decryption key to decipher the packet. The edge device will receive packet to complete application data transfer. The entire packet end-to-end latency is close to 30ms - a huge milestone in internet service in latency terms.

{% include video id="m05abdGSOxY" provider="youtube" %}

### Dispose
A smallsat life expectancy is about five years. Starting from 2024 hundreds of satellites will retire. The long-term sustainability of space programs requires a safe post-mission disposal of such satellites to avoid space debris. Inanticipation of space accidents, FCC in the US requires satellite companies to disclose debris mitigation plans [13] before launching satellites. The risk of collision with other objects and casualty risk on re-entry are the main concerns listed in FCC regulation.

Starlink disposes satellites by first deorbitting satellites from 550km altitude to 200 km. Next, the satellites re-enters the Earth’s atmosphere and burns. This plan is in early stages and will be increasingly stress tested in the next four years.

## Satellite constellation internet - A paradigm shift
Based on the current projections, the new LEO satellite constellations will **break barriers of low latency**. The internet packet latency is proportional to the distance between source and destination. The operating transatlantic cables in the ocean provide lowest latency between New Jersey and London which is a little under 60ms. The smallsat satellite internet shrinks the packet latency at least into half which is **30ms**. A uniform service across the globe with 30 ms latency breathes life into a wide variety of existing and new applications. A few published applications are:
...
Many unforeseen applications will be innovated when adoption spreads. The owners of the satellite constellations have the power to steer the direction of applications based on key parameters like price, location, and SLAs. They may limit the application to a small set like inter-datacenter connectivity, remote area internet, and automated driving or take up a global internet. The owners of satellite constellations have to cope with public expectations while realizing their business goals to deliver value to customers.

## Challenges
The road to the global internet has some predictable challenges and potentially unknown hurdles. The operating satellite constellation plans published in the public domain are aimed at sparsely populated areas. Additionally, satellite constellations have to withstand adverse weather conditions, round-the-clock operational challenges, web traffic surges, and competition. Scaling to global demand necessitates consensus with many countries and companies spread across the world most notably spectrum allocation in various countries.
